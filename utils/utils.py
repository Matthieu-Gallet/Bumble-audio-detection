...
def gen_srt(strlabel, onset, srtfile, duration=2, num=1):
    ...
    string_start = datetime.time(0, starttime // 60, starttime % 60).strftime(
        "%H:%M:%S"
    )
    ...
    with open(srtfile, "a") as f:
        ...
        f.write(
            "{starttime} --> {endtime}\n".format(
                starttime=string_start, endtime=string_end
            )
        )
        f.write("{}\n".format(strlabel))
        f.write("\n")
...
def read_audio_hdr(strWAVFile, verbose=False, file_format="wav"):
    if file_format == "wav":
        # Open file
        ...
        if "Recorded" in words[0]:
            ...
            curdate_time = datetime.datetime.strptime(
                str(words[3]) + str(words[2]), "%d/%m/%Y%H:%M:%S"
            )
            AMOid = words[7]
            if verbose:
            ...
            # Handle files with format like SMA02964-D05_20240408_083739.wav
            if "-" in strFile:
            ...
            curdate_time = datetime.datetime.strptime(
                "{}_{}".format(strFile[-2], strFile[-1][:-4]), "%Y%m%d_%H%M%S"
            )
            ...
            metadata = dict(
                time=curtime, date=curdate, datetime=curdate_time, id=SM4id, gain=0
            )

    else:
        strFile = os.path.basename(strWAVFile)
        # Handle files with format like SMA02964-D05_20240408_083739.flac
        if "-" in strFile:
            strFile = strFile.replace("-", "_")
        strFile = strFile.split("_")
        curtime = datetime.datetime.strptime(strFile[-1][:-5], "%H%M%S").time()
        curdate = datetime.datetime.strptime(strFile[-2], "%Y%m%d").date()
        curdate_time = datetime.datetime.strptime(
            "{}_{}".format(strFile[-2], strFile[-1][:-5]), "%Y%m%d_%H%M%S"
        )
