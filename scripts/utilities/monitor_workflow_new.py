...
class WorkflowMonitor:
        ...
        for item in os.listdir(self.data_path):
            ...
            if os.path.isdir(item_path) and not item.endswith("_annotées"):
                for subitem in os.listdir(item_path):
                    ...
                    if os.path.isdir(subitem_path) and not subitem.endswith(
                        "_annotées"
                    ):
        ...
        if evaluations:
            ...
            for eval_info in evaluations:
                if eval_info["has_metrics"] and "f1_score" in eval_info:
                    print(
                        f"  ✅ {eval_info['column']}: F1 = {eval_info['f1_score']:.3f}"
                    )
                else:
    ...
    def quick_status(self):
        ...
        if status["processed"] == 0:
            ...
            print(
                "  • Use continuous monitoring: python scripts/utilities/monitor_workflow.py --watch"
            )
        else:
            evaluations = self.check_evaluations()
            if not evaluations:
                ...
                print(
                    "  • Run evaluation: python scripts/workflow/final_batch_process.py"
                )
            else:
                print("\n💡 SUGGESTIONS:")
                print(
                    "  • Check detailed results: python scripts/utilities/check_results.py"
                )
