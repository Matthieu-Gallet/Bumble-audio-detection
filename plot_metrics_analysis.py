...
def create_plots(metrics_data, output_dir):
    ...
    opt_f1_trend = (
        "croissante"
        if df["optimal_f1"].iloc[-1] > df["optimal_f1"].iloc[0]
        else "décroissante"
    )
    roc_trend = (
        "croissante"
        if df["roc_auc"].iloc[-1] > df["roc_auc"].iloc[0]
        else "décroissante"
    )
    threshold_trend = (
        "croissante"
        if df["optimal_threshold"].iloc[-1] > df["optimal_threshold"].iloc[0]
        else "décroissante"
    )
...
def main():
    ...
    if not metrics_data:
        print(
            "Aucune donnée de métriques trouvée. Vérifiez que les évaluations avancées ont été exécutées."
        )
        return

    # Créer les graphiques
    analysis_dir = "~/Documents/Acoustique/detection/analysis"
